// ══════════════════════════════════════════════════════════════════════════════
//  geometry.cpp — RoboMower polygon mathematics
//
//  See geometry.h for public API and algorithm descriptions.
//
//  Float precision: all geometry uses float (ESP32-S3 hardware FPU).
//  Epsilons:
//    EPSILON_VERTEX   1e-4f  — vertex coincidence (0.1 mm)
//    EPSILON_PARALLEL 1e-6f  — line intersection denominator guard
//
...
/** Recursive worker — assumes the TOP-LEVEL input was normalised to CCW.
 *  Sub-polygons produced by splitting inherit their winding from the split:
 *  a CW-wound piece (signed area < 0) is a spur or fold — discard it. */
static std::vector<Polygon> splitSelfIntersectingRec(const Polygon &poly)
{
    ...
    // Signed area: CW (negative) ⇒ spur/fold piece from a parent split.
    float a = poly.area();
...
std::vector<Polygon> splitSelfIntersecting(const Polygon &poly)
{
    ...
    // split itself — a CW-wound but otherwise valid input (e.g. a perimeter
...
Polygon insetPolygon(const Polygon &poly, float inset_m)
{
    ...
    for (int i = 0; i < ne; ++i) {
        ...
        // valid point inside the polygon — using the intersection is correct and
...
// ══════════════════════════════════════════════════════════════════════════════
//  insetPolygonMulti — returns ALL sub-polygons (not just the largest)
// ══════════════════════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════════════════════════
//  smoothPolygonCorners — round convex corners to min turn radius
// ══════════════════════════════════════════════════════════════════════════════
...
Polygon smoothPolygonCorners(const Polygon &poly, float min_turn_radius)
{
    ...
    Polygon result;
    result.pts.reserve(n * 3);  // generous — arcs add points
    ...
    for (int i = 0; i < n; ++i) {
        ...
        // Normalise
        float u1x = d1x / len1, u1y = d1y / len1;  // unit A→B
        float u2x = d2x / len2, u2y = d2y / len2;  // unit B→C

        // Cross product: positive = left turn = convex corner in CCW polygon
        float cross = cross2(u1x, u1y, u2x, u2y);

        if (cross <= 0.0f) {
            // Concave corner (right turn) — keep sharp
            result.pts.push_back(B);
            continue;
        }
        ...
        if (turn_angle < 0.16f) {
            // Very gentle turn (~9°) — no smoothing needed
            result.pts.push_back(B);
            continue;
        }
        ...
        // For CCW polygon, the inward normal of edge A→B is (-u1y, u1x)
